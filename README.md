...
rollout, the store, eval, and wandb; ranks 1–3 only train. `--smoke` and
`--baseline` are single-process.

Rank 0 starts the rollout engine *before* `init_process_group`, with torchrun/elastic
env vars stripped for the spawn. Otherwise the TP=4 workers inherit `WORLD_SIZE=4` and
hang on torchrun's TCPStore (`127.0.0.1:<port>` timeout) instead of forming their own —
that is the Baseten 4+4 failure. Do not move `start()` to after the trainer group
joins. A local hint engine (`generator.hint.backend=vllm`) still starts first on its
own GPU and temporarily pins `CUDA_VISIBLE_DEVICES` to that device.
...
| GPUs               | 4 rollout / 4 trainer             | 8-GPU box; see below        |
| batch / mini-batch | 16 / 2                            | 16 divisible by 4 ranks     |
...
by worker index); each trainer rank pins `cuda:{n_rollout_gpus + rank}`. Policy keeps
TP=4. Startup fails if `torch.cuda.device_count()` is below the reserved count (8).
...
`generator.hint.reasoning_enabled` and `judge.reasoning_enabled`. Hints also
reserve 2,048 tokens for visible output (`generator.hint.max_tokens`).
OpenRouter counts hidden reasoning against the same budget, which otherwise
produces empty `finish_reason=length` responses.
...
The test suite is fully offline. Only the hint LLM / diligence judge / tau2 user sim /
Parallel search reach the network on a real run.
