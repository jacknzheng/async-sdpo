#!/usr/bin/env bash
# Diligence-bench SDPO on 9 GPUs (4 vLLM rollout + 4 FSDP trainer + 1 hint).
#
# Usage:
...
#
# Needs: 9 GPUs (4+4+1), PARALLEL_API_KEY (web search), OPENROUTER_API_KEY
#        (diligence judge), WANDB_API_KEY. Hints run on cuda:8 (Qwen3.5-9B).
